...
**   lfs.unlock (fh)
**
** $Id: lfs.c,v 1.1 2009-04-12 00:03:43 jasonsantos Exp $
*/
...
#ifdef _WIN32
#include <direct.h> /* IAN - use custom version */
//#include <io.h>  /* IAN */
#include <sys/locking.h> /* IAN - use custom version */
#ifdef __BORLANDC__
 #include <utime.h>
#else
 #include <sys/utime.h>
...
#define STAT_FUNC _wince_stati64		/* IAN */
#else
#ifdef _WIN32
 #ifdef __BORLANDC__
...
static int lfs_lock_dir(lua_State *L) {
  ...
  return 1;
#else
  luaL_error(L, "not implemented for Windows Mobile");
#endif
